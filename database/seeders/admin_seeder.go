...
func (s *AdminSeeder) Run() error {
	...
	exists, _ = facades.Orm().Query().Model(&models.Role{}).Where("slug", "demo").Exists()
	if !exists {
		...
		facades.Orm().Query().Where("slug", "demo").First(&demoRole)
	}
	...
	exists, _ = facades.Orm().Query().Model(&models.Admin{}).Where("username", "demo").Exists()
	if !exists {
