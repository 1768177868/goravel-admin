...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Foo":
		resolverName, err := entityResolverNameForFoo(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Foo": %w`, err)
		}
		switch resolverName {
		...
		case "findFooByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findFooByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindFooByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Foo": %w`, err)
			}
...
func entityResolverNameForFoo(ctx context.Context, rep EntityRepresentation) (string, error) {
	for {
		var (
		...
		return "findFooByID", nil
	}
	return "", fmt.Errorf("%w for Foo", ErrTypeNotFound)
}
