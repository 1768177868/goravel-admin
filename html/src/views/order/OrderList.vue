...
const handleSearch = () => {
  ...
  if (searchForm.start_time && searchForm.end_time) {
    const validation = validateTimeRange(searchForm.start_time, searchForm.end_time, ORDER_MAX_TIME_RANGE_MONTHS)
    if (!validation.valid) {
      ElMessage.warning(validation.error || t('order.time_range_exceeded'))
      return
    }
  }
