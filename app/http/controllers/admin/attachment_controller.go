...
// StreamUploadProgress SSE 实时推送文件上传进度
// 用于大文件分片上传时实时推送进度信息
func (r *AttachmentController) StreamUploadProgress(ctx http.Context) http.Response {
	// 获取参数
	chunkID := ctx.Request().Query("chunk_id", "")
	if chunkID == "" {
		return response.Error(ctx, http.StatusBadRequest, "chunk_id_required")
	...
	for {
		...
		case <-ticker.C:
			...
			progress, err := attachmentService.GetChunkProgress(chunkID, totalChunks)
			if err != nil {
				errorlog.RecordHTTP(ctx, "attachment", "Failed to get chunk progress in SSE", map[string]any{
					"error":    err.Error(),
					"chunk_id": chunkID,
			...
			// 检查进度是否有变化
			currentProgress := progress["progress"].(float64)
			currentUploadedCount := int(progress["uploaded_count"].(int))

			// 如果进度没有变化，跳过本次推送（但已完成状态需要推送）
			if currentProgress == lastProgress && currentUploadedCount == lastUploadedCount {
				// 如果已完成，需要推送完成消息
				if progress["completed"].(bool) && lastProgress >= 0 {
					continue
				}
				// 未完成且进度未变化，跳过
				continue
			...
			// 如果已完成，添加完成消息
			if progress["completed"].(bool) {
				message["type"] = "completed"
				message["message"] = "文件上传完成"
			}

			messageData, err := json.Marshal(message)
			if err != nil {
				errorlog.RecordHTTP(ctx, "attachment", "Failed to marshal progress", map[string]any{
					"error": err.Error(),
