...
func (r *MenuCreate) Rules(ctx http.Context) map[string]any {
	...
	if linkType == "2" {
		// 外部链接：需要验证为完整的 URL
		rules["path"] = "required|max:1000|url"
	} else {
		// 内部页面：只需要必填和长度验证
		rules["path"] = "required|max:1000"
		// 内部页面不验证 open_type
...
func (r *MenuCreate) Attributes(ctx http.Context) map[string]any {
