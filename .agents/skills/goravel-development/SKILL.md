...
- **`config/`** — One file per concern; each calls
 `facades.Config().Add(" ", map[string]any{...})` in `init()`. Read env
...
Facade return types are in `github.com/goravel/framework/contracts/ /`. For GitHub,
read the version in `go.mod` (e.g. `v1.18.0`) and browse
`https://github.com/goravel/framework/tree/v1.18.x/contracts/ `
...
Run `go run . artisan make: --help` to see flags and options for a
...
- Prefer `go test ` / `-run ` over `go test ./...` (slow).
